...
class LSTMSRT(nn.Module):
    def __init__(self, input_size, hidden_size, pose_size, num_layers=1, batch_first=False, grid_factor=3):
        super(LSTMSRT, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.batch_first = batch_first
        self.rnn = nn.LSTM(input_size, hidden_size, num_layers=num_layers, batch_first=False)
        self.srt_h_layers = nn.ModuleList([SpatialRecurrentTransformer(hidden_size, pose_size, grid_factor) 
                                           for _ in range(num_layers)])
        self.srt_c_layers = nn.ModuleList([SpatialRecurrentTransformer(hidden_size, pose_size, grid_factor) 
                                           for _ in range(num_layers)])
    ...
    def forward(self, x, pose, state=None):
        ...
        l, b, _ = x.shape
        if state is None:
            h0 = torch.zeros((self.num_layers, b, self.hidden_size), device=x.device)  # (num_layers, batch, hidden_size)
            c0 = torch.zeros((self.num_layers, b, self.hidden_size), device=x.device)  # (num_layers, batch, hidden_size)
            state = (h0, c0)
        ...
        for i in range(l):
            ...
            for j in range(self.num_layers):
                new_h = self.srt_h_layers[j](pose[i], h[j])
                new_c = self.srt_c_layers[j](pose[i], c[j])
                h_list.append(new_h)
                c_list.append(new_c)
            h = torch.stack(h_list, dim=0)
            c = torch.stack(c_list, dim=0)
            
            out, (h, c) = self.rnn(x[i:i+1], (h, c))
            out_seq.append(out)
...
if __name__ == '__main__':
    ...
    lstm_srt = LSTMSRT(input_size, hidden_size, pose_size, num_layers=2, batch_first=batch_first).to(device)
    
    out, state = lstm_srt(x, pose, None)
    print(f'Output shape: {out.shape}')
    print(f'State shape: {state[0].shape}', f'{state[1].shape}')
